module Flex
  ...
  class Scope < Variables
    ...
    METHODS = [:query, :terms, :filters, :sort, :fields, :size, :page, :params,
               :find, :first, :last, :all, :each, :destroy, :scan_all, :count]
    ...
    def find(ids, *vars)
      ...
      raise ArgumentError, "Empty argument passed (got #{ids.inspect})" \
            if wrapped.empty?
      result = Find::Scope.ids deep_merge(*vars, :ids => wrapped)
    ...
    def scan_all(*vars, &block)
      variables = deep_merge(*vars)
      Find::Scope.flex.scan_search(:scope, variables, &block)
    end
